# 千匣台 改进计划（第三轮 · 阶段 I–K · 2026-08-25）
...
>
> 工单池：QX-06 ~ QX-09 已登记 `docs/WORK_ORDERS.md`（本轮新增），按「独立分支 + 验收后合 main」派发。
...
| 3 | v1 政策封死本地导入 nodejs/python、dev 仅 client → **真机调用方只剩内置插件** | `WORK_ORDERS.md` QX-02 验收记录；`dev.ts:84-94` client-only |
...
| 5 | 内置 nodejs/python 插件有官方启动命令 + 桥凭据 env | `main.rs:108-149` `start_builtin_plugin`；`plugin_script.rs:605-609` 注入 `QIANXIA_PLUGIN_BRIDGE_URL/TOKEN` |
...
| 11 | QX-04b harness 绕过 SettingsPanel（凭据走 Rust env seam）→ **设置页用户旅程未真机补测** | `WORK_ORDERS.md` QX-04a 偏差记录 |
| 12 | CI `rust-tests` job 未真实 PR 验证；nightly E2E 未观察首周稳定性 | `IMPROVEMENT_PLAN.md` E3/E4 状态 |
| 13 | QX-04b 的 DeepSeek API key 曾在对话中出现 → 建议轮换 | `WORK_ORDERS.md` QX-04b 验收记录 ⚠️ |
| 14 | 流程纪律自 QX-05 恢复（独立分支 + 验收后合 main）→ 本轮延续 | `WORK_ORDERS.md` QX-05 记录 |
...
## 阶段 I：产品闭环验证（最高优先）→ 工单 QX-06

**目标**：把最后一条「代码已交付但真机未证」的链路（action 桥）跑成真机闭环；顺带补测 QX-04b 遗漏的
...
2. **内置 fixture 对**（build.rs 自动打包，见核实 #4）：
   ...
   - `builtin-plugins/action-caller/`（nodejs，entry `main.js`）：启动后**裸 fetch 直连桥**
     （`process.env.QIANXIA_PLUGIN_BRIDGE_URL + '/actions/call'` + `X-QianXia-Plugin-Token`，
...
**依赖/风险**：本机已有 runtimes 物料（QX-01/02 验证时 CDP 跑通）；nodejs 内置插件启动经
`start_builtin_plugin`（核实 #5）。风险：action manifest 依赖声明的确切形状需实现期对
`plugin-action.ts` 核对——列为实施第 1 步。

### I2 · SettingsPanel 用户旅程补测（QX-04b 遗留）
...
## 阶段 J：生态面补全（friction 剩余项）→ 工单 QX-07 / QX-08

**目标**：按 `docs/g2-sdk-friction.md` 剩余条目逐个收口；不做「为做而做」的项显式记录理由（J5）。

### J1 · storage 管理 API（friction #5 后半）→ QX-07
...
### J2 · AI 调用级 timeoutMs（friction #4）→ QX-08
...
### J3 · CLI `--quiet` 形态（friction #3）→ QX-08
...
### J4 · dev 自动重载防抖（QX-02-R 可选加固）→ QX-08
...
## 阶段 K：债务与治理 → 工单 QX-09 + 观察项

### K1 · 契约瘦身（H1，本轮执行）→ QX-09
...
共 ~63KB，核实 #10）；`@qianxia/contract` 移除对应 re-export；`pnpm-workspace.yaml` 已含
`packages/*`（无需改）；plugin-sdk / desktop 生产代码 import 为零（核实 #10）→ 改包低风险。
验证：三包 typecheck + vitest 全绿；grep 确认零残留 import。

**验收标准**：7 模块迁出；`@qianxia/contract` 只留桌面闭环需要的形状；全绿；PR 含工单号。
...
QX-04b 使用的 DeepSeek API key 曾在对话中出现（`WORK_ORDERS.md` QX-04b ⚠️ 记录）。**建议轮换**，
非代码任务；轮换后无需任何仓库改动（key 仅存在于环境变量/设置页）。

### K5 · 流程纪律（延续）

独立分支 + 验收后合 main（QX-05 已恢复）；本轮每工单按此执行。
...
| 工单 | 内容 | 建议 Agent | 依赖 |
|---|---|---|---|
| QX-06 | 阶段 I：I1 action 桥真机闭环 + 决策记录 + I2 SettingsPanel 旅程补测 | A | 本机 runtimes 物料 + WebView2 |
| QX-07 | J1 storage 管理 API（list/delete/count + 持久化修正） | A | 无 |
| QX-08 | J2 timeoutMs + J3 CLI --quiet + J4 dev 防抖（小件打包） | B | 无 |
| QX-09 | K1 契约瘦身（7 平台云模块迁出） | B | 无 |
...
1. **QX-06**（先立真机闭环：action 决策 + 桥闭环 + SettingsPanel 旅程；预计暴露缺陷最多，先做）；
2. **QX-07** 与 **QX-08** 并行（均为低风险增量；QX-08 依赖无）；
3. **QX-09**（纯搬包 + 验证，随时可插）；
