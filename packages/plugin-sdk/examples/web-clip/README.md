...
从系统剪贴板读取网页 URL（或手动粘贴），通过宿主 `net.fetch` 抓取页面正文，调用灵坊平台 LLM（`llm.chat`）生成中文摘要，最终将 URL、正文长度与摘要持久化到 `storage.kv`，并通过 `ui.view` 以宿主弹层展示结果。`client` 运行时示例插件，运行于桌面壳内嵌的沙箱 iframe 中，只通过宿主注入的 `window.sdk` 调用宿主能力，本身不持有任何平台密钥或令牌。

本插件是 LF-13「生态第二狗粮波」的第二个真实插件，刻意覆盖此前从未真机验证的 `clipboard.read` 与 `net.fetch` 两个 kind，并复用 `llm.chat` / `storage.kv` / `ui.view`。
...
### 4. `llm.chat`（灵坊平台 LLM 摘要）
- **数据访问**：将抽取出的正文（截断至 12k 字符）作为 `user` 消息、附系统提示「你是网页剪藏助手…」发送至灵坊平台 relay 的 `/llm/chat`，默认模型 `fast`。
