...
AI 能力面正向验证插件（LF-19 收口）。声明 `llm.chat` / `image.generate` / `video.generate` /
...
audio 若因 SDK 未接线而失败，属已如实标注的已知缺口（见 WORK_ORDERS LF-19 第 4 条）。

## 本地跑通

```bash
lingfang-plugin build packages/plugin-sdk/examples/relay-probe
```

配好 relay 凭据（设置页或 `LINGFANG_RELAY_API_BASE` / `LINGFANG_RELAY_TOKEN`）后
