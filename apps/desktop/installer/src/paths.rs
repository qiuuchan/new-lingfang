//! 安装目录定位（design §4）。
//!
//! 默认安装目录 `%LOCALAPPDATA%\QianXia`（保留旧路径，兼容旧版用户原地覆盖升级）。
...
/// 安装目录叶子名（保留旧 NSIS 路径 QianXia，非显示名「千匣」）。
pub const INSTALL_DIR_NAME: &str = "QianXia";

/// 应用显示名（开始菜单/快捷方式/控制面板展示）。
pub const DISPLAY_NAME: &str = "千匣台";

/// 版本号（编译期由 build.rs 从桌面端 tauri.conf.json 注入）。
/// 用于安装器标题/界面显示版本。
pub const VERSION: &str = env!("QIANXIA_APP_VERSION");

/// 主程序可执行文件名。
pub const MAIN_EXE: &str = "qianxia-desktop.exe";

/// 安装目录内的更新器/卸载器文件名（= installer.exe 的副本）。
pub const UPDATER_EXE: &str = "updater.exe";

/// 注册表 Uninstall 子键名（用应用标识，与旧 identifier 对齐）。
pub const UNINSTALL_KEY_NAME: &str = "com.qianxia.desktop";

/// 默认安装目录：`%LOCALAPPDATA%\QianXia`。
