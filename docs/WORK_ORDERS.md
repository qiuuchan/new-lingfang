...
## QX-01（建议派 Agent A）· G2 剪藏摘要狗粮插件 + SDK 摩擦记录
...
2. 走正式流程产出制品：`qianxia-plugin create`（client 模板）→ `validate` → `build` → `.qplugin` v4。
...
**验收标准**：
- [ ] `qianxia-plugin validate` + `build` 对示例插件干净通过，产物可被桌面壳本地导入并运行
...
## QX-02（建议派 Agent B）· G3 `qianxia-plugin dev` 热循环

**目标**：消灭插件作者「build→安装→重开」的高摩擦循环。两步：
- **v1 目录直读安装**：CLI 新增 `dev` 命令，把插件目录注册为 dev 安装（`origin='dev'`），
  `plugin_package_manager` 支持从目录直读（免打包 `.qplugin`），宿主内手动重开插件即刷新；
...
**验收标准**：
- [ ] `qianxia-plugin dev <dir>` 注册后，宿主可不经打包直接打开该插件（client 运行时）
...
## QX-03（第二轮派发，建议 Agent A 完成 QX-01 后接）· 文档还债组（G5 + H2 + H3 + F4）
...
## QX-05（2026-08-25 派发）· SDK 摩擦修复轮（g2-sdk-friction 驱动，Agent 自选）
...
## QX-06（建议派 Agent A）· action 桥真机闭环 + SettingsPanel 旅程补测（第三轮阶段 I）

**依据**：`IMPROVEMENT_PLAN_3.md` 阶段 I（核实结论 #1–#5、#11）。

**目标**：把最后一条「代码已交付但真机未证」的链路（client-action 桥）跑成真机闭环；补测 QX-04b
绕过设置页的偏差。
...
- [x] 真机闭环：caller `result.json` 含真实执行结果（非 `action_dependency_unresolved`）；反向对照稳定码
      —— `scripts/e2e-actions-verify.mjs` 真机跑通：`{"ok":true,"result":{"greeting":"hello qianxia"}}`
...
> 分支 `feat/qx-06-action-bridge`，待提 PR 引用 QX-06。

---

## QX-07（建议派 Agent A 完成 QX-06 后接）· storage 管理 API（friction #5 后半）
...
## QX-08（建议派 Agent B）· 小件打包：timeoutMs + CLI --quiet + dev 防抖（friction #4/#3 + QX-02-R 加固）
...
## QX-09（建议派 Agent B）· 契约瘦身（H1，本轮执行）
...
`@qianxia/contract` 移除对应 re-export；grep 确认桌面/plugin-sdk 零残留 import。

**验收标准**：7 模块迁出且 `@qianxia/contract` 只留桌面闭环形状；三包 typecheck + vitest 全绿；
grep 零残留；PR 含工单号。
...
`cargo test --workspace`、`pnpm typecheck`、`pnpm test` 全绿；PR 含工单号 QX-10。

**依赖**：无。注意 `updater.exe` update 模式首次真机运行，预期暴露集成缺陷（暴露即修，回写单测/runbook）。

---

## QX-11 · 干净机器安装 e2e + 新克隆一键灌装（阶段 L2+L3）
...
1. 新 `scripts/e2e-install-verify.mjs`（复用 CDP 与杀进程树惯例）：全新目标目录（无 runtimes 缓存 +
   隔离用户数据目录）跑 `QianXia-Setup-*.exe --silent --target` → 启动安装实例 → CDP 断言插件中心加载、
...
（操作前先备份现有 runtimes）；三基线全绿；PR 含工单号 QX-11。

**依赖**：无（可用 `v0.0.1-test` Release 产物或本机打包）。

---

## QX-12 · 能力面真机证据补齐（阶段 M）
...
1. `e2e-desktop-smoke.mjs` 增断言：`clipboard` writeText→readText 真机往返（clipboard.read 首次真机实证）；
   `storage.kv` list/delete/count 真机往返 + **delete 后重启不复活**（QX-07 落盘修正的真机证明）；
...
**验收标准**：新增 e2e 断言真机全绿；新增单测计入基线；三基线全绿；PR 含工单号 QX-12。
...
**目标**：第二个真实插件逼出未证明 kind 的手感问题；clip-digest 吃上 QX-07 新 API。
...
2. C2 ADR 回填：`docs/decisions/C2-relay-credential-source.md` 决策表回填「已采纳 C-on-A
   （2026-08-22 拍板，QX-04b 真机验证）」，状态 OPEN → 已采纳。
...
4. 分支清理（**需用户确认后执行**）：删除本地与远端 `qx-03-doc-debt`（已全量并入 main）。

**验收标准**：grep 复核五处过时消除；C2 ADR 状态与事实一致；`cargo build` 无 unused-import 警告；
纯文档/清理 diff，三基线不受影响；PR 含工单号 QX-14。
...
> 纪律不变：独立分支 + 验收后合 main，PR 描述引用工单号。
> 总闸：**QX-15（合并列车）先于一切**——其余工单全部建立在六分支并入 main 后的全树基线之上。
...
**范围**：
1. 按序合并：`chore/round-4-ledger` → `chore/qx-14-doc-debt` → `feat/qx-10-update-trigger` →
   `feat/qx-13-web-clip` → `feat/qx-11-install-e2e` → `feat/qx-12-cap-evidence`。
2. 已知冲突：`qx-14` 的 `f4d1728`（e2e 诊断增强）与 `qx-12` 均改 `scripts/e2e-desktop-smoke.mjs`，
   后合者解一次——以 qx-12 的重构（spawnShell/openNotes 复用）为主体，吸收 qx-14 的诊断输出增强。
3. 合并后全树回归：三基线 + `e2e-desktop-smoke` 真机跑一次（QX-12 断言集在合并树上复绿，
   特别注意 QX-10 `update.rs` 与 QX-07/QX-12 storage 断言同树）。
...
>    | 2 | chore/qx-14-doc-debt | `f5cd6f1` | `90374fc` |
>    | 3 | feat/qx-10-update-trigger | `60bd646` | `b80062b` |
>    | 4 | feat/qx-13-web-clip | `f17217a` | `78c71d1` |
>    | 5 | feat/qx-11-install-e2e | `18e59cd` | `2ba4fe5` |
>    | 6 | feat/qx-12-cap-evidence | `fda55ae` | `e410881` |
> 3. 预告的 smoke 脚本冲突被 git 三方自动合并消解、**零手工冲突**；已按工单要求做语义核对：
>    qx-12 的 `spawnShell`（:162）/`openNotes`（:181）重构与 qx-14 的 WebView2 进程父子诊断、
...
>    （diff 0 行）。
> 5. smoke 真机：debug 构建 + CDP **16 项断言全部通过**，含 QX-12 断言集
...
## QX-16 · 发版流水线补强：latest.json 上传 + 安装包签名

**目标**：补上 QX-10 链路真正可用前的最后一环——ADR `docs/decisions/update-feed-source.md` 已拍板
...
`check_update` 对该 feed 的解析有 fixture/单测证明；PR 含工单号 QX-16。

**依赖**：QX-15（update.rs 须在 main 上）。Org secret 配置需用户权限（用户行动项）。

---

## QX-17 · 更新链路真机闭环 e2e（QX-10 遗留，L1 第 4 点）

**目标**：兑现 QX-10 工单范围第 4 条——环回 update-feed 双向断言，这是更新链路唯一缺失的真机证据。
...
**验收标准**：双向断言真机绿且有记录；三基线全绿；PR 含工单号 QX-17。

**依赖**：QX-15。apply 段需安装实例环境，建议与 QX-18 同机连做。

> **🔧 已交付，待验收（2026-08-28，分支 `feat/qx-17-update-e2e`）——期间暴露并修复 1 个真实集成缺陷**
>
...
>
> **基线**：cargo **273 + installer 33** 全绿（回归跑于合并 main 后的 QX-17 分支）。
> e2e 脚本侧三处修复：`core.invoke` 路径、`quit_app` 替代 `window.close()`、CDP 首启导航竞态。

---

## QX-18 · 安装闭环本机复核（QX-11 遗留）

**目标**：兑现 QX-11 工单范围第 1 条的真闭环——「Release 安装器 → 干净目录 → 启动 → 插件可用」端到端记录。

**范围**：
1. **先重建安装器**：`target/release` 现存陈旧 0.1.11 产物，复用必假阳性——
   工单第一个动作是清理并用当前代码 `tauri build` 重新产出。
2. `QIANXIA_SETUP_EXE=<新构建> pnpm test:install` 真闭环：全新目标目录 + 隔离用户数据，
...
三基线不受影响；PR 含工单号 QX-18。

**依赖**：QX-15。执行环境 = 用户本机（脚本已就绪，开发机即可）。

> **🔧 已交付，待验收（2026-08-27，分支 `feat/qx-18-install-closed-loop`）——期间揪出并修复 2 个真缺陷**
>
> **范围第 1 条 ✅ 安装器重建**（陈旧 Aug23 产物已删，全部按当前代码重新产出）。
> **范围第 2 条 ✅ 安装闭环真机绿**：`QIANXIA_SETUP_EXE=<新构建> pnpm test:install` 全断言通过——
...
## QX-19 · 双插件导入 + 能力面观察项闭环（QX-13/QX-12 遗留）

**目标**：把第四轮留下的真机观察项一次性闭环或显式豁免，能力面证据不留「未验证」尾巴。

**范围**：
1. QX-13 遗留：clip-digest + web-clip 双 `.qplugin` 桌面壳导入走通
...
6. 与 QX-10 合并后含 update.rs 的全树回归——若 QX-15 已做，本条仅引用其结果不重复执行。

**验收标准**：1–4 有 CDP/脚本证据；5 有单测或豁免记录；三基线全绿；PR 含工单号 QX-19。

**依赖**：QX-15（插件与断言须在 main 上）。不被 QX-18 阻塞（可用 dev/debug 壳先行）。

> **🔧 已交付，待验收（2026-08-28，分支 `feat/qx-19-cap-closure`）**
>
> `scripts/e2e-cap-closure-verify.mjs` 真机全绿 exit 0（执行记录见
> `docs/verify-a5-client-plugin-e2e.md` U2 节）：
> 1) clip-digest + web-clip + relay-probe 三 `.qplugin` 导入 + F3 徽标「本地导入」+
...
> 来源：`IMPROVEMENT_PLAN_5.md`（第五轮，阶段 Q-T，QX-20~31）。
...
> 纪律不变：独立分支 + 验收后合 main，PR 描述引用工单号。
> 总闸：**阶段 P（QX-16~19）先行且必须验收通过（P5 闸门）**；Q-R-S-T 各工单按依赖表放行。

## QX-20 · README 重写 + Demo GIF（Q1+Q2，D2 拍板=必做）
...
**依赖**：P5（QX-16~19 验收通过）；R1 截图（QX-23 落地后补中段截图）。
...
> **✅ 已交付（2026-08-28，分支 `feat/qx-22-docs-facade`）**
>
...
## QX-23 · 真用插件 v1：本地知识库 / RAG（R1，D1=A 路径）
...
> **🔧 已交付，待验收（2026-08-28，分支 `feat/qx-23-kb-station`）**
>
...
> **🔧 已交付，待验收（2026-08-28，分支 `feat/qx-24-friction-feedback`）**
>
...
**依赖**：D3 拍板 ✅ + QX-23（产品可演示）。

> **🔧 物料已交付，触达待用户执行（2026-08-28，分支 `feat/qx-26-seed-outreach`）**
>
...
> → 收集 3-5 名种子 → ≥1 名跑通插件（核对清单打勾），完成后登记并衔接 QX-27。
...
## QX-28 · 公开 Release v0.1.11（S3，版本号 2026-08-28 拍板：沿用应用当前版本 0.1.11）
...
> assets 5 件齐全——`QianXia-Setup-0.1.11.exe`（633MB，minisign 签名）、`runtimes-bundle.zip`
> （1.7GB，签名）、`latest.json`（version 无 v 前缀 / sha256 / minisig_url 口径全对）。
> notes 完整：定位 + Demo GIF + 装机指引 + 已知限制 + 反馈通道（挂载自
> `docs/release-notes-v0.1.11.md`）。**QX-16 遗留验收（打 tag 实跑发版）随本 Release 一并闭环**。
...
## QX-29 · 内容输出持续（S4，D4 拍板=仅 GitHub Discussions）
...
## QX-31 · README metrics badges（T3）
...
- **QX-06 ✅ 验收通过（2026-08-25）**：action 桥真机闭环。分支 `feat/qx-06-action-bridge`
...
- **QX-07 ✅ 验收通过（2026-08-25）**：storage 管理 API。分支 `feat/qx-07-storage-mgmt`
...
- **QX-08 ✅ 验收通过（2026-08-25）**：分支 `feat/qx-08-timeout-quiet-debounce`。
  原提交 `d1c9b1a` 误并入 QX-06 的 plugin_runner 改动，经拆分手术重建为 `12dd30d`（纯 QX-08）：
  J2 timeoutMs（clamp [1000,180_000]，不泄漏桥参）、J3 CLI `--quiet`（四命令 + BuildError.path
  对齐）、J4 dev 重载 300ms 节流（should_emit_dev_reload）。
  ⚠️ 复验口径：plugin-sdk **146/146**（交付报告 150 含 QX-07 未合 main 的 4 测，基数差异非缺陷）、
  cargo 241/241、contract + desktop typecheck 干净；报告所称 App.tsx:214 typecheck 失败
  复验时不可复现（QX-06 WIP 中间态）。
  ⚠️ 流程教训：Agent 并发共享同一 checkout 导致跨工单 WIP 混入（QX-06 改动进 QX-08 提交、
  工作树互踩）。已用拆分手术修复；后续并发建议 per-agent worktree 或串行派发。

- **QX-09 ✅ 验收通过（2026-08-25）· 契约瘦身（H1）**：新建 `packages/platform-contract/`
 （`@qianxia/platform-contract`，依赖 `workspace:@qianxia/contract`），迁入 7 个平台云专属模块
  （marketplace-discovery / marketplace-commerce / plugin-governance / web-plugin-center /
  admin-governance / rbac / billing）及其 6 个 `.test.mjs`；`@qianxia/contract/src/index.ts`
  ...
  **依赖方向修正**：`plugin-registry.ts`（保留在 contract）原依赖 `admin-governance` 的
  ...
  验证：`@qianxia/contract` 37/37、`@qianxia/platform-contract` 34/34、plugin-sdk 150/150 全绿；
  apps/desktop typecheck 干净；grep 确认 7 模块零残留 import（仅 apps/desktop 内 "billing" 字符串标签）；
  pnpm-lock 已更新链接新包。PR 待提（引用 QX-09）。

- **QX-13 🟡 代码级验收通过（2026-08-25），真机 e2e 待补**：生态第二狗粮波。
  ...
  `com.qianxia.web-clip-0.1.0.qplugin`；含 SSRF 拦截提示与 relay_not_configured 降级。
  2) clip-digest 自修：删除 kv set 静默兜底 localStorage，改用 QX-07 list/delete/count 做 LRU 淘汰 +
  ...
  2026-08-26 已提交分支 `feat/qx-13-web-clip`（`f17217a`，8 文件，diff 纯净仅本工单）。
  ⚠️ 待补项：真机 e2e（WebView2 桌面壳导入两个 .qplugin 的 CDP 证据）本环境不可跑，需用户本机复核；
  clip-digest 修复的真机复验同。
  （原「工作区并存 QX-10 半成品」警告已消解：QX-10 已提交 `feat/qx-10-update-trigger`，
  SettingsPanel.tsx:177 不可达比较由验收人修复，见 QX-10 条目。）

- **QX-14 ✅ 代码级验收通过（2026-08-26）**：文档债清理 + 警告收敛。分支 `chore/qx-14-doc-debt`
  （未推远端）。逐项核验属实：
  ...
  WebView2 父子进程归属 + runtime 版本上报），非 QX-14 范围。**处置：保留在分支内，PR 描述注明**。
  （原「QX-10 两处红」来自当时未提交的工作区文件，已随 QX-10 提交修复，不在本分支 diff 内。）

- **QX-10 🟡 代码级验收通过（2026-08-26），真机 e2e 待补**：应用侧更新触发链路。分支
  `feat/qx-10-update-trigger`（`60bd646`，6 文件，diff 纯净仅本工单）。
  1) ADR `docs/decisions/update-feed-source.md`：feed = GitHub Releases `latest.json`，验签复用
  `verify_minisign`，relay 托管备选记录解除条件；
  2) `update.rs` 三命令 + `get_app_version`（build.rs 注入 `QIANXIA_APP_VERSION`，与 installer 同源），
  main.rs 注册；
  ...
  typecheck）；`DEFAULT_FEED_URL` 原指向不存在的 `qianxia/workbench-releases`，与 ADR「本仓库」
  语义矛盾，改对齐 `qiuuchan/new-qianxia`。
...
- **QX-11 🟡 代码级验收通过（2026-08-26），安装器真闭环待本机**：干净机器安装 e2e + 一键灌装。
  分支 `feat/qx-11-install-e2e`（`18e59cd`，5 文件，diff 纯净仅本工单）。
  1) `scripts/populate-local-runtimes.mjs`：本地优先（QIANXIA_RUNTIME_BUNDLE > 已校验幂等跳过）
  + gh 远程回退（minisign 验签，QIANXIA_RUNTIME_PUBKEY 同信任根）+ --force 备份式重灌与失败回滚
  ...
  pnpm test 全绿。
  验收人修复（含在 `18e59cd`）：远程回退默认仓库 `qianxia/desktop` → `qiuuchan/new-qianxia`；
  `gh release download latest` 字面量会被当成 tag 名，改为不传 tag 取最新 Release。
  ⚠️ 交付报告称「cargo 预存失败 `main.rs:566 extract_host_keeps_ipv6_brackets`（QX-10 半成品）」
  **复验不复现**：该测试属 QX-12 提交 `37c9ae6`（非 QX-10），当前树单测 4/4 通过、全量
  263+30 全绿——疑似交付 agent 跑在 QX-12 未完成的中间态。
  ⚠️ 待补项：SFX 安装器 `--silent` 真闭环（`QIANXIA_SETUP_EXE=... pnpm test:install`）+
  远程回退重灌路径，需具备 Release 与 `QIANXIA_RUNTIME_PUBKEY` 的本机执行。

- **QX-12 ✅ 验收通过（2026-08-26，含真机证据）**：能力面真机证据补齐。分支
  `feat/qx-12-cap-evidence`（交付 `37c9ae6` + 验收人修复 `fda55ae`，共 11 文件）。
  ...
  **验收人真机复跑**（2026-08-26，本环境 WebView2 CDP 闭环可用——近期 e2e 诊断修复链已生效，
  ...
  storage list/delete/count 真机往返、delete 后重启不复活（QX-07 落盘修正真机证明）、
  clipboard/net.fetch 未声明网关负向。
  **验收人修复（`fda55ae`）**：
  ① **QX-07 遗留宿主缺陷**——PluginRunner 注入 iframe 的 CLIENT_SDK_BOOTSTRAP 漏同步
  storage list/delete/count（iframe 内 `window.sdk.storage.list` 不存在，npm SDK 与单测全绿、
  真机 TypeError；QX-13 clip-digest/web-clip 的 LRU 路径真机必踩）。补齐三方法并对齐 npm 门面
  ...
  clipboard 的插件——QX-13 web-clip 合入后可闭环）；net.fetch 公网 200；relay image/video/audio
  四 kind 正向（audio SDK 尚未接线）；is_blocked_host 真实域名 DNS fail-closed；
  与 QX-10 合并后的全树回归。

- **QX-01 ✅ 验收通过（2026-08-24）**：validate/build 干净、制品 v4 结构正确；`pnpm typecheck`/`pnpm test`（256）
  ...
  ⚠️ 流程偏差：改动未走独立分支/PR，直接留在 main 工作区（含 QX-02 改动混在一起）。
- **QX-02 ⬜→🔧 返工修复已提交（QX-02-R）**：watch 启动点已补到 `load_installed_plugin`（`commands.rs:30`），`cargo build` 通过；待真实桌面闭环复验自动重载。详见下节。
- **QX-02 ✅ 验收通过（2026-08-24，QX-02-R 复验闭环）**：桌面 CDP 实测全绿——注册/账本完整性（dev 1 + builtin 3）/目录直读打开/sdk 注入/能力注册（system.info 真实返回）/**改文件后 iframe 自动重载（新 marker 生效）**/重载后能力可用。cargo 267 + vitest 259 + typecheck 全绿。

### QX-02-R · 返工项：client dev 插件 watch 永不启动
...
**缺陷证据（桌面 CDP 实测，release 产物含 QX-02 全部改动——修复前）**：
...
**验收口径（QX-02-R 通过标准）**：在真实桌面闭环（非单元测试）下，dev 插件打开 → 修改源文件 →
...
重载后能力可用；`unregister_dev_dir` 幂等注销成功。QX-02 正式验收通过。

---

## QX-03 / QX-04a 验收记录

- **QX-03 ✅ 验收通过（2026-08-24）**：G5 前瞻节符号化（残留行号仅存于「0.核实结论」表与「附录」——历史证据区，属存档豁免）；
...
- **QX-04a ✅ 验收通过（2026-08-24）**：`require_relay` 凭据优先级 用户设置 > `QIANXIA_RELAY_API_BASE`/`QIANXIA_RELAY_TOKEN`，
  ...
  Rust 侧 env 直读 seam——设置页路径未被 harness 覆盖，留待 QX-04b 人工补测该用户旅程。
- **QX-04b ✅ 验收通过（2026-08-24）**：用户提供 DeepSeek API key（仅经环境变量注入）+ 新增本地 relay 适配器
...
- **QX-05 ✅ 验收通过（2026-08-25）**：plugin-sdk vitest 135（+9：code-first 降级 ×2、relay 归一 ×2、
  ...
  `feat/qx-05-sdk-friction` 提交 `6fbcb2c`；产品确认后已合入 main（`b854a9b`）。
...
| 轮次 | Agent A | Agent B |
|---|---|---|
| 第 1 轮 | QX-01（G2 狗粮插件） | QX-02（G3 dev 热循环） |
| 第 2 轮 | QX-03（文档还债组） | QX-04a（G1 前置脚本，凭据未到前） |

QX-04b 待凭据到位后单独派发。任何工单在验收前均不合并 main。
