...
1. **token 来源**：用户手动配置（A）——不引入账号概念。
2. **client 直连 relay**：禁止（C）——新增 host 代理命令，凭据不进 iframe。
3. **client 无 AI 能力**：不接受 D——`notes` 等 client 插件 AI 功能经代理命令可用。
4. **计费/配额归属**：沿用 bridge 既有团队灵石计费模型，client 调用归属同一账户。
